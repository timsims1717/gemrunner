	...
	for _, result := range myecs.Manager.Query(myecs.IsDynamic) {
		...
		if okO && okC && !obj.Hidden &&
			...
			if !ch.Flags.NoCollision {
				...
				var eLeft, eRight, eUp, eDown bool
				eLeft, eRight, eUp, eDown = entityCollision(ch, px, py)
				if pTile == nil {
					...
					leftWallCollisions(ch, pTile, pLeft, eLeft, chPos)
					rightWallCollisions(ch, pTile, pRight, eRight, chPos)
					ceilingCollisions(ch, pTile, pUp, eUp, chPos)
					floorCollisions(ch, pTile, pDown, eDown, chPos)
					chPos = ch.Object.Pos
					x, y := world.WorldToMap(chPos.X, chPos.Y)
					if x != px || y != py { // check again if they changed tiles
						setCollisionFlags(ch)
						eLeft, eRight, eUp, eDown = entityCollision(ch, x, y)
						...
						leftWallCollisions(ch, tile, left, eLeft, chPos)
						rightWallCollisions(ch, tile, right, eRight, chPos)
						ceilingCollisions(ch, tile, up, eUp, chPos)
						floorCollisions(ch, tile, down, eDown, chPos)
					}
				}
			} else {
				pPos := ch.Object.LastPos
				px, py := world.WorldToMap(pPos.X, pPos.Y)
				ch.Flags.LeftWall, ch.Flags.RightWall, ch.Flags.Ceiling, ch.Flags.Floor = entityCollision(ch, px, py)
			}
		}
	}
}
...
func leftWallCollisions(ch *data.Dynamic, tile, left *data.Tile, entityLeft bool, chPos pixel.Vec) {
	...
	if ch.State == data.OnLadder || ch.State == data.Falling {
		ch.Flags.LeftWall = left.IsSolid() || left.Block == data.BlockLiquid || (left.Block == data.BlockFall && !left.Flags.Collapse) || entityLeft
		...
		} else if entityLeft && ch.Object.Pos.X < ch.Object.LastPos.X {
			ch.Flags.LeftWall = true
...
func rightWallCollisions(ch *data.Dynamic, tile, right *data.Tile, entityRight bool, chPos pixel.Vec) {
	// for left and right, we stop the character if the next tile is solid and either
	//   if they run into the tile
	//   or if they are on a ladder (so they stay in the center of the ladder)
	...
	if ch.State == data.OnLadder || ch.State == data.Falling {
		ch.Flags.RightWall = right.IsSolid() || right.Block == data.BlockLiquid || (right.Block == data.BlockFall && !right.Flags.Collapse) || entityRight
	} else if chPos.X+ch.Object.HalfWidth >= tile.Object.Pos.X+world.HalfSize {
		if right.IsSolid() || right.Block == data.BlockLiquid || (right.Block == data.BlockFall && !right.Flags.Collapse) {
			ch.Flags.RightWall = true
			ch.Object.Pos.X = tile.Object.Pos.X + world.HalfSize - ch.Object.HalfWidth
		} else if entityRight && ch.Object.Pos.X > ch.Object.LastPos.X {
			ch.Flags.RightWall = true
			ch.Object.Pos.X = ch.Object.LastPos.X
		}
	}
...
func ceilingCollisions(ch *data.Dynamic, tile, up *data.Tile, entityUp bool, chPos pixel.Vec) {
	...
	if chPos.Y+ch.Object.HalfHeight >= tile.Object.Pos.Y+world.HalfSize {
		...
		} else if entityUp && ch.Object.Pos.Y > ch.Object.LastPos.Y {
			ch.Flags.Ceiling = true
			ch.Flags.Climbed = SetClimbed(ch)
...
func floorCollisions(ch *data.Dynamic, tile, down *data.Tile, entityDown bool, chPos pixel.Vec) {
	...
	if ch.Flags.NoLadders {
		...
		if entityDown && ch.State == data.Falling && touchingFloor {
			ch.Object.Pos.Y = tile.Object.Pos.Y - world.HalfSize + ch.Object.HalfHeight
		} else if entityDown && ch.State == data.OnLadder {
			if touchingFloor {
...
//func entityCollision(ch *data.Dynamic, x, y int) (bool, bool, bool, bool) {
//	if ch.Enemy < 0 {
