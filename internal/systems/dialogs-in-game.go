	...
	if dialog, ok := ui.Dialogs[key]; ok {
		...
		case constants.DialogPlayer1Inv,
			...
			if dialog.Key != constants.DialogPuzzleTitle {
				...
				dialog.ViewPort.Canvas.SetUniform("uRedGoop", float32(1))
				dialog.ViewPort.Canvas.SetUniform("uGreenGoop", float32(1))
				dialog.ViewPort.Canvas.SetUniform("uBlueGoop", float32(1))
