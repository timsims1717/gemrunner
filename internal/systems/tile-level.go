		...
		if okO && ok && data.CurrLevel.Start && tile.Live {
			...
			case data.BlockBarrier:
				if reanimator.FrameSwitch && data.CurrLevel.DoorsOpen && tile.Metadata.Regenerate {
					tile.Counter++
					if tile.Metadata.Toggle == tile.Flags.Collapse &&
						tile.Counter > constants.FrameCycle*(tile.Metadata.RegenDelay+1) {
						if tile.Metadata.Toggle {
							tile.Flags.Collapse = false
							tile.Flags.Cracked = false
							tile.Counter = 0
							AddMaskWithTrigger(tile, "open_block_mask", false, true, func() {
								RemoveMask(tile)
								tile.Counter = 0
							})
							CrushCharacters(tile)
							...
							tile.Flags.Cracked = false
							tile.Counter = 0
							AddMaskWithTrigger(tile, "open_block_mask", false, false, func() {
								//RemoveMask(tile)
								tile.Block = data.BlockEmpty
								tile.Flags.Cracked = false
								tile.Counter = 0
